...
static void	determineCropSize(t_cropRange *crop, t_object *object, int height, int width)
{
	...
	if ((crop->maxY - crop->minY > MAX_CROP_SIZE)
		&& (CROP_SIZE_LIMIT == 1))
	{
		crop->minY = object->centerY - MAX_CROP_SIZE / 2;
		crop->maxY = object->centerY + MAX_CROP_SIZE / 2;
	}
	if ((crop->maxX - crop->minX > MAX_CROP_SIZE)
		&& (CROP_SIZE_LIMIT == 1))
	{
		crop->minX = object->centerX - MAX_CROP_SIZE / 2;
...
int exportObject(t_image *image, t_object *object, char *filename)
{
	...
	if (return_value = copyPixelData(fd, cropRange, newFilename(filename, object->label)) != RETURN_SUCCESS)
		return (return_value);
